(function ($) {
  $(document).ready(function () {
    ...
    function uploadImage() {
      ...
      uploader.on('change', function () {
          ...
          reader.onload = function(event) {
            images.append('<div id="perf-fotos" style="background-image: url(\'' + event.target.result + '\');" rel="'+ event.target.result  +'"><span id="del1">Borrar</span></div>')
          }
          reader.readAsDataURL(uploader[0].files[0])

       })

      $('#del1').on('click', '#perf-fotos', function () {
        $(this).remove()
      })

    }
    function perfilpic() {
      var button = $('#upperfpic')
      var uploader = $('<input type="file" accept="image/*" />')
      var images = $('#perfpic')
      var del = $('#del2')
      ...
      uploader.on('change', function () {
          var reader = new FileReader()
          reader.onload = function(event) {
            images.html('<div id="perfpic" style="background-image: url(\'' + event.target.result + '\');" rel="'+ event.target.result  +'"><span id="del2">Borrar</span></div>')
            let image ={};
    ...
    $('#btnedit').on('click', function(){
        ...
        document.location.href='/perfiledit';
    })
